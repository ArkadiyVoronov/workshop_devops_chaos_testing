...
def inject_failures():
    ...
    if state['memory_leak_mb'] > 0:
        ...
        max_leak = 200 * 1024 * 1024  # Максимум 200 MB
        if current_leak < max_leak:
            _leaked_memory.append(bytearray(leak_size))

@contextmanager
def db_transaction():
    """Контекстный менеджер для отслеживания активных транзакций БД"""
